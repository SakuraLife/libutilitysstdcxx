  ...
  namespace memory
  {
    ...
    class unique_ptr
    {
      public:
        ...
        typedef typename
          __detail::__unique_ptr_pointer<_T, _Deleter>::type
          pointer;
    ...
    {
      template<typename _T>
      struct __check_unique_type
      { typedef unique_ptr<_T> __single;};

      template<typename _T>
      struct __check_unique_type<_T[]>
      { typedef unique_ptr<_T[]> __unbound_array;};

      template<typename _T, size_t _N>
      struct __check_unique_type<_T[_N]>
      { typedef void __bound_array;};
    }

    template<typename _T, typename... _Args>
    inline typename __detail::__check_unique_type<_T>::__single
    make_unique(_Args&&... _args)
    {
      using algorithm::forward;
      return unique_ptr<_T>(new _T(forward<_Args>(_args)...));
    }
    template<typename _T, typename... _Args>
    inline typename __detail::__check_unique_type<_T>::__unbound_array
    make_unique(size_t _len)
    {
      using algorithm::forward;
      typedef trait::type::transform::remove_extent_t<_T> _U;
      return unique_ptr<_T>(new _U[_len]());
    }
    template<typename _T, typename... _Args>

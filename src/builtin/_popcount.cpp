...
namespace
{
#ifdef UTILITY_LARGE_LOOKUP_TABLE
  const static int HW[65536] =
  {
  ...
  template<>
  struct __popcount<32>
  {
    template<typename _T>
    static inline int aux(_T _val) noexcept
    { return HW[(_val >> 16) & 0xffffU] + HW[_val & 0xffffU];}
  };

  template<>
  struct __popcount<64>
  {
    template<typename _T>
    static inline int aux(_T _val) noexcept
    {
      return HW[(_val >> 48) & 0xffffU]
           + HW[(_val >> 32) & 0xffffU]
           + HW[(_val >> 16) & 0xffffU]
           + HW[_val & 0xffffU];
    }
  ...
  template<>
  struct __popcount<128>
  {
    template<typename _T>
    static inline int aux(_T _val) noexcept
    {
      return HW[(_val >> 112) & 0xffffU]
           + HW[(_val >> 96) & 0xffffU]
           + HW[(_val >> 80) & 0xffffU]
           + HW[(_val >> 64) & 0xffffU]
           + HW[(_val >> 48) & 0xffffU]
  ...
  struct __popcount<16>
  {
    template<typename _T>
    static inline int aux(_T _val) noexcept
    { return HW[static_cast<_T>(_val >> 8U) & 0xffU] + HW[_val & 0xffU];}
  };
...
#endif // ! UTILITY_LARGE_LOOKUP_TABLE
